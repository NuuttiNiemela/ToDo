function getRequest() {
    var resultElement = document.getElementById('tehtavat')
    ...
    axios.get('http://localhost:8080/api/todo/')
        .then(function (response) {
            for(var t of response.data) {
                if(t.tarkempiTehtava===null || t.tarkempiTehtava.trim()==='') t.tarkempiTehtava = 'Ei lisätietoja';
                if (!Boolean(t.valmis)) {
                    resultElement.innerHTML +=  '<tr>' + '<td id="rivi' + t.id + '"><span id="alkuperainen' + t.id + '">' + t.tehtava + '</span><span class="muokkausClass" id="muokkaus' + t.id + '"><input class="muokkausTekstiClass" id="muokkaaTehtava' + t.id + '" type="text" value="' + t.tehtava + '"/>' + '&nbsp&nbsp&nbsp' + '<button id="muokkausSubmit" onclick="muokkaus(' + t.id + ')">Submit</button></span>' +
                        ...
                        '<br><br><span class="lisatiedot" id="more' + t.id + '">' + t.tarkempiTehtava + '</span>' + '<span class="muokkausClass" id="muokkausLisa' + t.id + '"><input class="muokkausTekstiClass" id="muokkaaLisaTehtava' + t.id + '" type="text" value="' + t.tarkempiTehtava + '"/></span>' + '</td>' + '</tr>'
                    jaljessa(t.id, t.aikataulu)

                } else if (Boolean(t.valmis)) {
                    valmiitElement.innerHTML += '<tr>' + '<td><span id="alkuperainen' + t.id + '">' + t.tehtava + '</span>' +
                        '<div id="toiminnot">' + '<span class="aikatauluClass" id="aikatauluId' + t.id + '">' + paivamaara(t.aikataulu)+ '&nbsp&nbsp&nbsp&nbsp&nbsp' + kellonAika(t.aikataulu) + '</span>' +
                        '<button id="nuoli' + t.id + '" onclick="lisaaTietoa(' + t.id + ')">▼</button><button onclick="eiSittenkaanTehty(' + t.id + ')">-</button><button onclick="delRequest(' + t.id + ')">Delete</button></div>' + '<br><br><span class="lisatiedot" id="more' + t.id + '">' + t.tarkempiTehtava + '</span>' + '</td>' + '</tr>'
                }
            }
        })
...
function muokkaus(e) {
    var resultElement = document.getElementById('postResult')
    var uusi = document.getElementById('muokkaaTehtava' + e).value
    var lisatiedot = document.getElementById('muokkaaLisaTehtava' + e).value
